...
class Terrier(Engine):
    ...
    def get_document(self, document_id):
        ...
        #try: content == self.__reader.getItem("body", int(idx))
        #except: content = "NA"
        #content = self.__reader.getItem(self.text_field, int(idx))
        try: content = self.__reader.getItem(self.text_field, int(idx))
        except: content = "NA" 
        #try: content = self.__reader.getItem(self.text_field, int(document_id))
        #try: content = self.__reader.getItem("body", int(document_id)) 
        #except: content = "NA"
        #content= "Testtesttesttest"
        #content = self.__text 
        #try: title = self.__reader.getItem("headline", int(document_id))
        try: title = self.__reader.getItem(self.title_field, int(idx))
        except: title = "problem with title in get_document "
    ...
    @staticmethod
    def _parse_terrier_response(self, response, title_field='title', text_field='body'):

        #filename = "./queries_llama_irreldoc_wapo.txt"
        #with open(filename, 'a') as file:
        #    file.write(response["query"].iloc[0] + "\n")

        output = Response(response['query'].iloc[0])
        for i in range(len(response)):
            row = response.iloc[i]
            #log.debug(row, "ROWROW new")
            #log.debug(f"Row _parse_terrier_response: {type(row)}")
            title_field = "title"
            title = getattr(row, title_field, "NA")
            #log.debug(f"Title _parse_terrier_response: {title}")
            #log.debug(f"Title _parse_terrier_response: {row.values}")
            #log.debug(f"Title _parse_terrier_response: {row.index}")
            #title = "Nicht na"
            url = getattr(row, 'url', row.docno)
            idx = self.__reader.getDocument('docno', row.docno)
            try: title = self.__reader.getItem(self.title_field, int(idx))
            except: title = "problem with title in get_document "
            content = getattr(row, text_field, None) 
            #content = "Kleiner test in parse_terrier"
            rank = row['rank'] + 1
            docid = row.docno 
            score = row.score
            source = row.source
    ...
    def _request(self, query):
        ...
        if self.__engine:
            ...
            response = self._parse_terrier_response(self, response)
        return response
