...
class LangChainTextClassifier_based_on_contrastive_Context_no_trec(BaseTextClassifier):
    """
    ...
    def __init__(self, topic, user_context, prompt_file, prompt_file_old, result_type_str, provider = 'ollama', model = 'mistral', temperature = 0.0, verbose = False):
        ...
        """
        super(LangChainTextClassifier_based_on_contrastive_Context_no_trec, self).__init__(topic, user_context)
        self.updating = False
        prompt_template = ""
        ...
        self._prompt = PromptTemplate(
            ...
            input_variables=["topic_title", "doc_title", "doc_content","summarized_relevant_documents", "summarized_irrelevant_documents"],
            partial_variables={"format_instructions": format_instructions})
        
        self._prompt_old = PromptTemplate(
            template=self._template_old,
            input_variables=["topic_title", "doc_title", "doc_content"],
            partial_variables={"format_instructions": format_instructions}
        )
    ...
    def is_relevant(self, document):
        ...
        if relevant_summary is None and irrelevant_summary is None:
            out = self._llm_old.generate_response(self._output_parser,{ 'topic_title': topic_title, 'doc_title': doc_title, 'doc_content': doc_content},self._result_schema)
        elif relevant_summary is None and irrelevant_summary is not None:
            out = self._llm.generate_response(self._output_parser,{ 'topic_title': topic_title, 'doc_title': doc_title, 'doc_content': doc_content ,'summarized_relevant_documents':"None found yet", 'summarized_irrelevant_documents':irrelevant_summary},self._result_schema)
        elif relevant_summary is not None and irrelevant_summary is None:
            out = self._llm.generate_response(self._output_parser,{ 'topic_title': topic_title, 'doc_title': doc_title, 'doc_content': doc_content ,'summarized_relevant_documents':relevant_summary, 'summarized_irrelevant_documents':"None found yet"},self._result_schema)
        else:
            out = self._llm.generate_response(self._output_parser,{ 'topic_title': topic_title, 'doc_title': doc_title, 'doc_content': doc_content ,'summarized_relevant_documents':relevant_summary, 'summarized_irrelevant_documents':irrelevant_summary},self._result_schema)
