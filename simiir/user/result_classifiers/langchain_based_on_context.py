...
class LangChainTextClassifier_based_on_Context(BaseTextClassifier):
    """
    ...
    def __init__(self, topic, user_context, prompt_file, prompt_file_old, result_type_str, provider = 'ollama', model = 'mistral', temperature = 0.0, verbose = False):
        ...
        """
        super(LangChainTextClassifier_based_on_Context, self).__init__(topic, user_context)
        self.updating = False
        prompt_template = ""
        ...
        self._prompt = PromptTemplate(
            ...
            input_variables=["topic_title", "topic_description", "doc_title", "doc_content","summarized_documents"],
    ...
    def is_relevant(self, document):
        ...
        summarized_documents = self._user_context.get_summary_of_documents()
        
        if summarized_documents is not None:
            out = self._llm.generate_response(self._output_parser,{ 'topic_title': topic_title, 'topic_description': topic_description, 'doc_title': doc_title, 'doc_content': doc_content ,'summarized_documents':summarized_documents},self._result_schema)
        else:
