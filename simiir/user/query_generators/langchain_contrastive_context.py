...
class LangChainQueryGenerator_based_on_contrastive_Context(BaseQueryGenerator):
    """
    ...
    def __init__(self, stopword_file, prompt_file, n=3,provider='ollama',model='mistral',temperature=1.0,verbose=False, background_file=[]):
        super(BasicLangChainQueryGenerator_based_on_constrastive_Context, self).__init__(stopword_file, background_file=background_file, allow_similar=False)
        prompt_template = ""
        ...
        self._prompt = PromptTemplate(
            ...
            input_variables=["topic_title", "topic_description","summarized_relevant_documents", "summarized_irrelevant_documents"],
    ...
    def generate_query_list(self, user_context):
        ...
        if relevant_summary is None and irrelevant_summary is None:
            ...
            out = self._llm.generate_response(self._output_parser,{ 'topic_title': topic_title, 'topic_description': topic_description, 'summarized_relevant_documents':"None found yet", 'summarized_irrelevant_documents':irrelevant_summary},self._result_schema)
        elif relevant_summary is not None and irrelevant_summary is None:
            out = self._llm.generate_response(self._output_parser,{ 'topic_title': topic_title, 'topic_description': topic_description, 'summarized_relevant_documents':relevant_summary, 'summarized_irrelevant_documents':"None found yet"},self._result_schema)
        else:
            out = self._llm.generate_response(self._output_parser,{ 'topic_title': topic_title, 'topic_description': topic_description, 'summarized_relevant_documents':relevant_summary, 'summarized_irrelevant_documents':irrelevant_summary},self._result_schema)
