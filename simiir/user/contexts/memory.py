...
import logging
from utils.rel_doc_summarizer import LangChainDocumentSummarizer
...
class Memory(object):
    ...
    """
    def __init__(self, search_interface, output_controller, topic, summary_boolean=False, summary_Type="contrastive", query_after_SERP=False):  
        """
        ...
        self.topic = topic
        self.summary_boolean = summary_boolean  # Determines if a summary is generated
        self.summary_Type = summary_Type        # Determines the type of summary generated
        self.query_after_SERP = query_after_SERP# Determines if a query is reformulated after a SERP or taken from existing list of queries
        ...
        self._last_summary = None               # The last summary generated by the user
        self._last_rel_doc_summary = None       # The last relevant document summary generated by the user
        self._last_irrel_doc_summary = None     # The last irrelevant document summary generated by the user
    ...
    def _set_snippet_action(self):
        ...
        snippet = Document(result.whooshid, result.title, result.content, result.docid)
    ...
    def get_irrelevant_documents(self):
        """
        Returns the list of documents marked irrelevant throughout the simulation.
        """
        return self._irrelevant_documents
        
    def get_doc_summary(self):
        """
        Returns the summary of the relevant documents.
        """
    ...
    def get_rel_doc_summary(self):
        """
        Returns the summary of the relevant documents.
        """
        if self._last_rel_doc_summary is None:
            return None
        
        return self._last_rel_doc_summary
    
    def get_irrel_doc_summary(self):
        """
        Returns the summary of the irrelevant documents.
        """
        if self._last_irrel_doc_summary is None:
            return None
        
        return self._last_irrel_doc_summary
    
    def get_summary_of_irrelevant_documents(self):
        """
        Generates a summary of the irrelevant documents using a language model.
        """
        if self.summary_boolean == True:
            if self.summary_Type == "irrelevant" or self.summary_Type == "contrastive":
                #docs = self._irrelevant_documents
                docs = self.get_irrelevant_documents()

        if not docs:
            return None
        #if len(docs) > 5:
        #    text_to_summarize = " ".join([doc.content for doc in docs[-5:]])
        #else:
        text_to_summarize = " ".join([doc.content for doc in docs])


        summarizer = LangChainDocumentSummarizer()
        summary = summarizer.generate_summary_string(text_to_summarize)
  
        if self.summary_Type == "irrelevant":
            self._last_summary = summary
        else:
            self._last_irrel_doc_summary = summary
        return summary
    
    def get_summary_of_relevant_documents(self):
        """
        Generates a summary of the relevant documents using a language model.
        """
        if self.summary_boolean == True:
            if self.summary_Type == "relevant" or self.summary_Type == "contrastive":
        ...
        if self.summary_Type == "relevant":
            self._last_summary = summary
        else:
            self._last_rel_doc_summary = summary
        return summary
    

    def get_summary_of_documents(self):
        """
        Generates a summary of the relevant documents using a language model.
        """
        ...
        if self.summary_boolean == True:
            if self.summary_Type == "irrelevant":
                docs = self.get_irrelevant_documents()
            elif self.summary_Type == "relevant":
                docs = self.get_relevant_documents()
            if not docs:
                return None
